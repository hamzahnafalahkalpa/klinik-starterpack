...
class EmployeeSeeder extends Seeder
{
    ...
    {
        $users = ['admin','bidan','cashier','dokter','farmasi','frontoffice','laboran','radiologi','perawat'];
        foreach ($users as $user_data) {
            $user = app(config('database.models.User'))->where('username',$user_data)->first();
            if (!isset($user)){
                ...
                request()->merge([
                    ...
                    "people" => [ // Informasi individu
                        "id" => null,
                        "name" => "hamzah nur al falah - ".$user_data,
                    ...
                    "user_reference" => [ // Referensi user
                        ...
                        "user" => [ // Informasi akun user (boleh null untuk tidak update akun user)
                            ...
                            "email" => "hamzah nur al falah$user_data@dev.com",
                ...
                app(config('app.contracts.Employee'))
                    ->prepareStoreEmployee($this->requestDTO(app(config('app.contracts.EmployeeData'))));
            }
        }
    }
}
